...
class PageServiceTest {
    ...
    void getDetailPageNull() {
        ...
        List<CafeResponseDto> cafeList = new ArrayList<>();
        List<Comment> commentList = new ArrayList<>();
        commentList.add(comment);

        when(cafeRepository.findByBeanAndCafeAddressContaining(bean, "no address")).thenReturn(cafeList);
        ...
        // when
        ResponseEntity<Message> response = pageService.getDetailPage(beanId, "", 1);
    ...
    @Test
    @DisplayName("getDetailPage address = 서울 강남구")
    void getDetailPageAddress() {
        ...
        when(cafeRepository.findByBeanAndCafeAddressContaining(bean, "서울 강남구")).thenReturn(cafeList);
        when(commentRepository.findByBean(bean,1)).thenReturn(commentList);
        when(commentRepository.findByBean_id(bean.getId())).thenReturn(commentList);
        when(utilService.checkBean(beanId)).thenReturn(bean);

        // when
        ResponseEntity<Message> response = pageService.getDetailPage(beanId, "서울 강남구", 1);
        DetailPageResponseDto responseDto = (DetailPageResponseDto) Objects.requireNonNull(response.getBody()).getData();
    ...
    void setDetailPageCafeNull() {
        ...
        // when
        List<CafeResponseDto> ResponseCafeList = pageService.setDetailPageCafe(bean, "null");
    ...
    @Test
    @DisplayName("setDetailPage address = 서울 강남구")
    void setDetailPageCafeAddress() {
        // given
        Bean bean = new Bean();
        List<CafeResponseDto> cafeList = new ArrayList<>();

        when(cafeRepository.findByBeanAndCafeAddressContaining(bean, "서울 강남구"))
                .thenReturn(cafeList);

        // when
        List<CafeResponseDto> ResponseCafeList = pageService.setDetailPageCafe(bean, "서울 강남구");
